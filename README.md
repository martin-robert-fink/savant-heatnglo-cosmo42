...
- **Climate tile** — the fireplace as a heater: ambient temperature, heat setpoint, and two modes. **Heat** ignites and holds the setpoint; **Off** extinguishes. Savant's `Auto` is deliberately not declared — it means auto-changeover, which implies cooling and makes Blueprint draw a dead Cool button. To burn regardless of temperature, use the trigger service's On/Off tile
...
- **Thermostat** — setpoint in whole °F or °C, plus up/down nudges and off. Disabling it means writing a zero setpoint to the appliance, so the bridge keeps a copy of your temperature and restores it when you return to Heat
